...
class ResNetDiff(nn.Module):
    def __init__(self, block, layers, num_classes):
        self.inplanes = 64
        super(ResNetDiff, self).__init__()
        ...
        for m1, m2 in zip(self.layer5.conv2d_list, self.layer6.conv2d_list):
            for (n1, w1), (n2, w2) in zip(m1.named_parameters(), m2.named_parameters()):
                ...
                if 'weight' in n1:
                    ...
                    # orthogonal feature
                    # diff += (w1 * w2).sum([1, 2, 3]).norm(norm).pow(norm)

                    # feature select
                    # diff += (w1 * w2).norm(norm).pow(norm)

                    # channel select, orthogonal kernel
                    # diff += (w1 * w2).sum([2, 3]).norm(norm).pow(norm)
        ...
        for cn, (m1, m2) in enumerate(zip(self.layer5.conv2d_list, self.layer6.conv2d_list)):
            for (n1, w1), (n2, w2) in zip(m1.named_parameters(), m2.named_parameters()):
                ...
                if 'weight' in n1:
                    ...
                    conflict = vec.abs()
                    # -1, 1, K1, K2
                    c_conflict = vec.sum([1], keepdim=True).abs()
                    # -1, C, 1, 1
                    k_conflict = vec.sum([2, 3], keepdim=True).abs()
                    # -1, 1, 1, 1
                    ck_conflict = vec.sum([1, 2, 3], keepdim=True).abs()

                    conflicts = torch.cat([torch.cat([conflict, c_conflict], axis=1).reshape(-1, C+1, K1*K2),
                                           torch.cat(
                                               [k_conflict, ck_conflict], axis=1).reshape(-1, C+1, 1),
...
def DeeplabDiff(num_classes=21):
    model = ResNetDiff(Bottleneck, [3, 4, 23, 3], num_classes)
    return model
