...
def main():
    ...
    with torch.no_grad():
        for index, batch in tqdm(enumerate(testloader), total=len(testloader)):
            ...
            per_class_max_output1 = list(map(lambda c: max_output1.flatten()[argmax_output1.flatten() == c], range(19)))
            per_class_max_output2 = list(map(lambda c: max_output2.flatten()[argmax_output1.flatten() == c], range(19)))
            ...
            threshold_output1 = torch.cat([max_output1.flatten().sort()[0], torch.tensor([1.], device=gpu0)])[threshold_idx_output1]
            threshold_output2 = torch.cat([max_output2.flatten().sort()[0], torch.tensor([1.], device=gpu0)])[threshold_idx_output2]
            per_class_threshold_output1 = torch.stack(list(map(
                lambda c: torch.cat([per_class_max_output1[c].sort()[0], torch.tensor([1.], device=gpu0)])[per_class_threshold_idx_output1[c]],
                range(19))))
            per_class_threshold_output2 = torch.stack(list(map(
                lambda c: torch.cat([per_class_max_output2[c].sort()[0], torch.tensor([1.], device=gpu0)])[per_class_threshold_idx_output2[c]],
                range(19))))
            ...
            for idx, p in enumerate(range(0, 100, 10)):
                ...
                if 'class' in args.thtype: 
                    ...
                    if args.pstype == '1': output_ = per_class_filter_output1
                    elif args.pstype == '2': output_ = per_class_filter_output2
                    elif args.pstype == 'and': output_ = per_class_filter_output_and
                    elif args.pstype == 'or': output_ = per_class_filter_output_or
                    ...
                    if args.pstype == '1': output_ = filter_output1
                    elif args.pstype == '2': output_ = filter_output2
                    elif args.pstype == 'and': output_ = filter_output_and
                    elif args.pstype == 'or': output_ = filter_output_or

                prefix = os.path.join(args.save, '_'.join([args.thtype, args.pstype]), name.split('_')[0], '_'.join(name.split('_')[:-1])) + '_psLabel'
                ...
                output.save(prefix + '_labelIds.png')
